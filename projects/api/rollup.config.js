...
const config = {
    ...
    plugins: [
        ...
        isWatch &&
            run({
                execArgv: ["--require=../../.pnp.cjs", "--require=source-map-support/register", "--no-warnings"],
            }),
        ...
        !isWatch && {
            ...
            async writeBundle() {
                ...
                await fs.writeFile("dist/.gcloudignore", ".pnp.*\n.yarn/*\n!.yarn/patches\n!.yarn/plugins\n!.yarn/releases\n.gcloudignore\nenv.yml\n");
            },
        },
    ],
    ...
    onwarn(warning, warn) {
        if (!(warning.id?.includes("node_modules") || warning.message?.startsWith("Unknown CLI flags: env."))) {
            warn(warning);
        }
    },
};
